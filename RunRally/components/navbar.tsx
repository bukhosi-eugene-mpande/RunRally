        ...
        <ul className="hidden lg:flex gap-4 justify-start ml-2">
          {siteConfig.navItems.map((item) => (
            <NavbarItem key={item.href} className='text-3xl font-semibold'>
              <Link color="foreground" href={item.href} className='text-3xl font-semibold text-black'>
                {item.label}
              </Link>
            </NavbarItem>
          ))}
      ...
      >
        ...
        <NavbarItem className="hidden sm:flex gap-2">
          <div className="relative py-2 mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#000000" className="file: mt-4 h-10 w-10">
